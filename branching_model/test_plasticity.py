...
def test_adaptation(doses, plot_title, plot_f, n_steps=100, learning_rate=10**-1):
    ...
    pheno_df = pd.DataFrame(phenotypes, columns=["S", *[f"R{i}" for i in range(1, Cell.N_TREATMENTS+1)]])
    pheno_df["Time"] = time_array
    plot_pheno_df = pheno_df.melt(id_vars="Time", var_name="Trait", value_name="Expression")
...
def test_treatment(plot_title, plot_f, n_steps=100, learning_rate=10**-1):
    cell = Cell.Cell(0, learning_rate=learning_rate)
    phenotypes = [None] * 3*n_steps
    no_treatment_doses = torch.zeros(Cell.N_TREATMENTS).reshape(1, -1)
    for i in range(n_steps):
...
no_treatment_doses = torch.zeros(Cell.N_TREATMENTS).reshape(1, -1)
test_adaptation(doses=no_treatment_doses, plot_title=no_treament_title, plot_f=no_treament_f)
